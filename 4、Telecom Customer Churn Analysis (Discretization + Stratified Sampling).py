...
#2、处理量纲差异大（连续变量）
#（1）特征离散化
print(df[['tenure','MonthlyCharges','TotalCharges']].describe())#了解数据分布情况，要用列表双括号来选择多列
churn_var['tenure'] = pd.qcut(churn_var['tenure'],4,labels=['1','2','3','4'])
#qcut函数将连续变量按分位数来转换为分类变量，4表示4个区间
print(churn_var['tenure'].head())
...
print(churn_var['MonthlyCharges'].head())
print(churn_var['TotalCharges'].head())
#将数据类型转换为整数类型
churn_var['tenure'] = churn_var['tenure'].cat.codes + 1
#.cat.codes 处理category类型数据，转换为整数编码，从0开始所以加1，这里会保持原始分位数的数据
churn_var['MonthlyCharges'] = churn_var['MonthlyCharges'].cat.codes + 1
churn_var['TotalCharges'] = churn_var['TotalCharges'].cat.codes + 1
print(churn_var.dtypes)
print(churn_var['TotalCharges'].head())
#3、分类数据转换为整数编码
...
#（1）分层抽样
print(churn_var.dtypes)
print(list(map(label,churn_var.columns)))#确认都改成数值了
x = churn_var
y = df['Churn']
sss = StratifiedShuffleSplit(n_splits=5,test_size=0.2,random_state=0)
#一般1000以下是小数据集，1000-1万属于中等，1万以上是大数据集，小数据集建议10次，中大型建议3-5次
print(sss)
